...
clean_long <- named_right_data %>%
  ...
  separate(info, into = c("temp", "dateplus"), sep = "(?<=_)(.*)", remove = FALSE) #%>% 
  separate(dateplus, into = c("temp", "dateplus"), sep = "(?!.*_)(?<=_)", remove = FALSE)
# remove tailing "-" from new ID and date 
clean_long$ID <- substr(clean_long$ID, 1, nchar(clean_long$ID)-1)
clean_long$date <- substr(clean_long$date, 1, nchar(clean_long$date)-1)
...
\
Below is Julia's code for combining and cleaning all C and N data
